...
class queryBuilder {
	...
	function queryBuilder($file, $tpl, $ext) {				/* На входе ссылка на валидный файл Excel и на массив-шаблон мапинга столбцов и параметров процедуры */	
		...
		$this->query = " IF (LEN(@procResult) > 1) \n"
					. "begin \n"
					. "insert into @resultTable(stat, mess) values ('Не обработан', 'Процедура вставки завершилась с ошибкой - ' + @procResult); \n"
					. "end else \n"
					. "begin \n"
					. "insert into @resultTable(stat, mess) values ('Обработан', null); \n"
					. "end; ";
		$table = '';
		$lastrow = '';
		$isTable = 0;
		for ($i = 1; $i <= $sheet->getHighestRow()-1; $i++) {
			...
			for ($j = 1; $j < $nTpl; $j++) {
					if($isTable <= $nTpl){ 																			/* Формируем строку создания временной таблицы согласно шаблона*/
						...
						if($j == $nTpl-1){							
							$table =
							"DECLARE @resultTable TABLE (stat varchar(20), mess varchar(500) null); \n"
							. "DECLARE @procResult varchar(max) \n";
			...
			$this->value[$i] =
			"BEGIN TRY "
			."exec ".$tpl[0][0]." ".$this->value[$i]." @result = @procResult OUTPUT;"
			. "END TRY "
			. "BEGIN CATCH "
			. "insert into @resultTable(stat, mess) values('Не обработан', ERROR_MESSAGE());"
			. "END CATCH"
			.$this->query;
		...
		if ($this->builded == true/* || 1 == 1*/){
			$this->query = $table. $all_query . "; SELECT * FROM @resultTable";

Ext.define('fplk.controller.WebWbsCont', {
    ...
    init: function () {
        this.control({
            ...
            'webwbstool button[action=WebWbAll]': {
                click: this.fireAllButtonClick
            },
            'webwbstool button[action=WebWbAdded]': {
                click: this.fireAddedClick
            },
            'webwbstool button[action=WebWbNotAdded]': {
                click: this.fireNotAddedClick
            }
        });
    },

    /**
     * Показывает ВЕБ НАКЛАДНЫЕ, в которых отсутсвует накладная.
     * СОБЫТИЕ - Нажатие на кнопку "Не внесена"
     * @param button кнопка "Не внесена"
     */
    fireNotAddedClick: function (button) {
        this.toggleGroupButton(button);
        var grid = this.getWebWbsGrid();
        var store = grid.getStore();
        store.clearFilter();
        store.filter([
            Ext.create('Ext.util.Filter', {
                filterFn: function (item) {
                    return item.get('web_count') === 0;
                }, root: 'data'
            })
        ])
    },
    ...
    /**
     * Показывает ВЕБ НАКЛАДНЫЕ, в которых есть накладная.
     * СОБЫТИЕ - Нажатие на кнопку "Внесена"
     * @param button кнопка "Внесена"
     */
    fireAddedClick: function (button) {
        this.toggleGroupButton(button);
        var grid = this.getWebWbsGrid();
        var store = grid.getStore();
        store.clearFilter();
    ...
    /**
     * Показывает все ВЕБ НАКЛАДНЫЕ.
     * СОБЫТИЕ - Нажатие на кнопку "Все"
     * @param button кнопка "Все"
     */
    fireAllButtonClick: function (button) {
        this.toggleGroupButton(button);
        var grid = this.getWebWbsGrid();
        var store = grid.getStore();
        store.clearFilter();
    ...
    /**
     * Выполняет переключение между кнопками группы "Внесение накладных"
     * Выключает все, кроме активированной.
     * !!! ВАЖНО !!! Кнопки должны иметь атрибут "action"
     *
     * @param button Активированная кнопка
     */
    toggleGroupButton(button) {
        button.toggle(true);
        var group = button.up('buttongroup');
        Ext.Array.each(group.items.items, function (bt) {
            if (bt.action !== button.action) {
                bt.toggle(false);
            }
        })
    },
    ...
    /**
     * Экспорт в Excel.
     * Действие - *Нажатие на кнопку "Экспорт в Excel"*
     */
